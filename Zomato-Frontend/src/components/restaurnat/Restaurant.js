...
export default function Restaurant() {
  ...
  let makePayment = async () => {
    ...
    var options = {
      ...
      image: "https://upload.wikimedia.org/wikipedia/commons/2/2d/Zomato_Logo.jpg",
      order_id: order.id,
  ...
  let getMenuList = async () => {
    let URL = "http://localhost:4000/api/get-menuitems?resid=" + params.id;
    try {
      let response = await axios.get(URL);
      let data = response.data;
    ...
    <>
        ...
        <div className="modal-dialog  modal-fullscreen ">
          <div className="modal-content bg-black">
            <div className="d-flex justify-content-between">
              <h5 className="modal-title text-light ms-3 pt-1" id="staticBackdropLabel">
                {rDetails.name}
              </h5>
              <button type="button" className="btn-close text-light me-3 fs-3 border-0" data-bs-dismiss="modal" aria-label="Close">
                ×
              </button>
            </div>
                ...
                {rDetails.thumb.map((value, index) => {
                  return (
                    <div key={index} className="h-100 w-100">
                      <img src={"../image/" + value} alt="image loading..." className="h-75 w-50 restaurant-gallery mt-2" />
                    </div>
                  );
                })}
              </Carousel>
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h4 className="modal-title nav-blue fw-bold" id="exampleModalToggleLabel">
                {rDetails.name}
              </h4>
            ...
            <div className="modal-body">
              {menuItem.map((menuItems, index) => {
                return (
                  <div className="row border-bottom border-2 p-1 py-3" key={menuItems._id}>
                    <div className="col-8 small">
                      <p className="mb-1 fw-bold h6 nav-blue">{menuItems.name}</p>
                      <p className="mb-1">{menuItems.price}</p>
                      <p className="small text-muted">{menuItems.description}</p>
                    </div>
                    <div className="col-4 d-flex justify-content-end mb-3">
                      <div className="menu-food-item">
                        <img src={`../image/${menuItems.image}`} alt="" />
                        {menuItems.qty === 0 ? (
                          <button className="btn btn-primary btn-sm" onClick={() => incMenuitemCounter(index)}>
                            Add
                          </button>
                        ) : (
                          <div className="order-item-count add">
                            <span className="span-button text-danger" onClick={() => decMenuitemCounter(index)}>
                              -
                            </span>
                            <span>{menuItems.qty}</span>
                            <span className="span-button text-success" onClick={() => incMenuitemCounter(index)}>
                              +
                            </span>
                          </div>
                        )}
              ...
              {subTotal === 0 ? null : (
                <div className=" d-flex justify-content-between mt-2">
                  ...
                  <button className="btn btn-danger bg-danger" data-bs-target="#exampleModalToggle2" data-bs-toggle="modal">
                    Pay Now
                  </button>
                </div>
              )}
      ...
      <div className="modal fade" id="exampleModalToggle2" aria-hidden="true" aria-labelledby="exampleModalToggleLabel2" tabIndex="-1">
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header">
              <h4 className="modal-title nav-blue fw-bold" id="exampleModalToggleLabel2">
                {rDetails.name}
              </h4>
              <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            ...
            <div className="modal-footer">
              <button className="btn btn-danger bg-danger" onClick={makePayment}>
                PROCEED
              </button>
            </div>
          </div>
            ...
            <div className="col-12">
              ...
              <div className="d-flex justify-content-between mt-4">
                <ul className="list-unstyled d-flex gap-4 nav-blue fw-bold mb-0 cursor-pointer">
                  <li className={isContact === true ? null : "border-bottom border-3 border-danger "} onClick={() => setIsContact(false)}>
                    Overview
                  </li>
                  <li className={isContact === false ? null : "border-bottom border-3 border-danger"} onClick={() => setIsContact(true)}>
                    Contact
                  </li>
                </ul>
                <button
              ...
              {isContact === false ? (
                <div className="over-view mt-3">
                  ...
                  <p className="text-muted">{rDetails.cuisine.reduce((pv, cv) => pv + ", " + cv.name, "").substring(2)}</p>

                  <p className="nav-blue mt-3 fw-bold mb-0">Average Cost</p>
                  <p className="text-muted">{rDetails.min_price} for two people (approx.)</p>
                </div>
              ) : (
